...
import { searchBookstore } from '../../api/booksnap.api';
...
const SearchBookstoreModal = ({ setModalOpen, selected, setSelected }: ModalProps) => {
  const [bookstoreOptions, setBookstoreOptions] = useState<Option[]>([]);
  ...
  useEffect(() => {
    searchBookstore(null).then((data) => {
      const mappedOptions: Option[] = data.data.map((item: any) => ({
        label: item.bookStoreName,
        value: String(item.bookStoreId),
        bookStoreName: item.bookStoreName,
  ...
  const handleCreate = (inputValue: string) => {
    const newOption: Option = {
      bookstoreId: Date.now(),
      bookStoreName: inputValue,
      label: inputValue,
      value: inputValue,
    };
