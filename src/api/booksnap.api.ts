...
// 작가로 책 검색
export const getSearchBookByAuthor = async (searchWord: string, page: number) => {
  try {
    const response = await instance.get(`api/booksnap/book-search-by-author?query=${searchWord}&page=${page}`);
    if (response.status == 200) {
      return response.data;
    }
  } catch (error) {
